...
public static unsafe partial class StreamlineAPI
{
    ...
    public static void LoadPCLFunctions()
    {
        ...
        fixed (byte* name = "slPCLSetMarker\0"u8)
        {
            ...
            Console.WriteLine($"[Streamline] slGetFeatureFunction(slPCLSetMarker) returned: {(Result)res}, pointer: {(IntPtr)func:X}");
        }
    ...
    {
        if (type == 0) return; // Skip info logs
    ...
    public static int EarlyInitStreamline()
    {
        try
        {
            ...
            {
                Console.WriteLine($"[Streamline] slInit early initialization succeeded.");
            }
            return initRes;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[Streamline] Failed to load or initialize sl.interposer.dll during early init: {ex.Message}");
            return -1;
        }
    }
}
