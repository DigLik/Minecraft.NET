    ...
    D3D12Context d3d) : IRenderPipeline
{
    ...
    public void Initialize()
    {
        ...
        {
            RangeType = DescriptorRangeType.Srv, NumDescriptors = 1, BaseShaderRegister = 0, RegisterSpace = 0, OffsetInDescriptorsFromTableStart = 0
        };
        ...
        var sampler = new StaticSamplerDesc
        {
            ...
            BorderColor = StaticBorderColor.TransparentBlack, MinLOD = 0.0f, MaxLOD = float.MaxValue,
        ...
        {
            NumParameters = 3, PParameters = parameters, NumStaticSamplers = 1,
            PStaticSamplers = &sampler, Flags = RootSignatureFlags.AllowInputAssemblerInputLayout
        };
        ...
        psoDesc.RasterizerState = new RasterizerDesc
        {
            ...
            SlopeScaledDepthBias = 0, DepthClipEnable = 1, MultisampleEnable = 0, AntialiasedLineEnable = 0, ForcedSampleCount = 0,
    ...
    private bool IsBoxInFrustum(Vector3 min, Vector3 max)
    {
        for (int i = 0; i < 6; i++)
        {
            ...
            Vector3 axisVert = new Vector3(
                p.X < 0.0f ? min.X : max.X,
                p.Y < 0.0f ? min.Y : max.Y,
                p.Z < 0.0f ? min.Z : max.Z
            );

            if (Vector3.Dot(new Vector3(p.X, p.Y, p.Z), axisVert) + p.W < 0.0f)
                return false;
    ...
    public void OnRender(double deltaTime)
    {
        ...
        FrameConstants frameConstants = new FrameConstants
        {
            ...
            WireframeColor = new Vector4(0, 0, 0, 1), UseWireframeColor = renderSettings.IsWireframeEnabled ?
        ...
        int drawCalls = 0;
        const int MaxDrawCallsPerFrame = 8000;
        ...
        for (int i = 0; i < chunks.Count; i++)
        {
            ...
            if (!IsBoxInFrustum(chunkMin, chunkMax))
                continue;
        ...
        if (hr < 0)
        {
            ...
            if (hr == unchecked((int)0x887A0005))
            {
                int removedReason = d3d.Device.Get().GetDeviceRemovedReason();

                Console.WriteLine($"[GPU CRASH] Device Removed Reason: {removedReason:X}");
            }
            Environment.Exit(-1);
