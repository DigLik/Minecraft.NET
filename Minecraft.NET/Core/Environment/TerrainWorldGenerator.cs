...
public class TerrainWorldGenerator : IWorldGenerator
{
    ...
    public unsafe void Generate(ChunkSection column)
    {
        Random random = new Random(column.Position.X * 73856093 ^ column.Position.Z * 1920164449 ^ 12345);
                ...
                for (int y = 0; y <= height; y++)
                {
                    BlockId block = y == height
                        ...
                        : y > height - 4
                            ? block = BlockId.Dirt
                            : block = BlockId.Stone;
                    column.SetBlock(new(x, y, z), block);
                }
            }
    }
}