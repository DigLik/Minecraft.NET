...
public unsafe class VulkanRenderPipeline : IRenderPipeline
{
    ...
    // НОВАЯ СТРУКТУРА: Передаем адреса видеопамяти напрямую в шейдер!
    ...
    private void CreateStorageImage()
    {
        ...
        ImageViewCreateInfo viewInfo = new() { SType = StructureType.ImageViewCreateInfo, Image = _storageImage, ViewType = ImageViewType.Type2D, Format = Format.B8G8R8A8Unorm, SubresourceRange = new ImageSubresourceRange(ImageAspectFlags.ColorBit, 0, 1, 0, 1) };
    ...
    public void RenderFrame(CameraData cameraData)
    {
        ...
        if (_drawCalls.Count > 0)
        {
            ...
            for (int i = 0; i < _drawCalls.Count; i++)
            {
                ...
                instSpan[i] = new AccelerationStructureInstanceKHR { Transform = tf, InstanceCustomIndex = (uint)i, Mask = 0xFF, InstanceShaderBindingTableRecordOffset = 0, Flags = GeometryInstanceFlagsKHR.ForceOpaqueBitKhr, AccelerationStructureReference = alloc.BlasDeviceAddress };

                // Передаем прямые адреса в памяти видеокарты
        ...
        if (_drawCalls.Count > 0)
        {
            ImageCopy copy = new() { SrcSubresource = new ImageSubresourceLayers(ImageAspectFlags.ColorBit, 0, 0, 1), DstSubresource = new ImageSubresourceLayers(ImageAspectFlags.ColorBit, 0, 0, 1), Extent = new Extent3D((uint)_framebufferSize.X, (uint)_framebufferSize.Y, 1) };
