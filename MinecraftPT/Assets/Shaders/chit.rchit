...
layout(binding = 5, set = 0, scalar) readonly buffer Materials { MaterialData m[]; } materials;
...
void main() {
    ...
    vec3 e1 = p1 - p0;
    vec3 e2 = p2 - p0;
    vec3 geomNormal = normalize(cross(e1, e2));
    bool isFrontFace = dot(geomNormal, gl_WorldRayDirectionEXT) < 0.0;
    vec3 normal = isFrontFace ? geomNormal : -geomNormal;
    ...
    payload.albedo = (mat.type == 2.0) ? mix(vec3(1.0), texColor.rgb, texColor.a) : texColor.rgb;
    payload.metallic = mat.metallic;
    payload.emission = texColor.rgb * mat.emission;
    payload.opacity = (mat.type == 2.0) ? mix(mat.opacity, 1.0, texColor.a) : 1.0;
    payload.ior = mat.ior;
