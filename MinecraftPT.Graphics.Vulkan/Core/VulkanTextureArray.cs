...
public unsafe class VulkanTextureArray : ITextureArray
{
    ...
    public VulkanTextureArray(VulkanDevice device, int width, int height, byte[][] pixels)
    {
        ...
        {
            if ((ulong)pixels[i].Length < layerSize)
                throw new ArgumentException($"Pixel buffer for layer {i} (size {pixels[i].Length} bytes) is smaller than the expected size ({layerSize} bytes).");
